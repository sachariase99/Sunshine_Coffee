...
const Cart = ({ cartItems, isCartOpen, toggleCart, decreaseItem, increaseItem }) => {
  ...
  return (
    <div className={`fixed top-0 right-0 bottom-0 w-[50%] bg-[#EFEFEF] z-20 font-inika ${isCartOpen ? '' : 'hidden'}`}>
      <div className="text-black flex flex-col items-center mt-32">
        ...
        <div className='flex items-center justify-between w-1/2 mt-32'>
          ...
          <Link to="/checkout" className="px-8 bg-black text-white py-4 rounded-lg mb-12 mt-6">Go to Checkout</Link>
        </div>
      </div>
    </div>
  );
